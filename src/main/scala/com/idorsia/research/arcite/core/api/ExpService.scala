    ...
    post {
      entity(as[SearchExperiments]) { gexp ⇒
        ...
        val exps: Future[SomeExperiments] =
          expActor.ask(SearchExperiments(gexp.search, gexp.maxHits)).mapTo[SomeExperiments]
        onSuccess(exps) { fe ⇒
          complete(OK -> fe)
        }
      }
      ...
      parameters('page ? 0, 'max ? 100) { (page, max) ⇒
        ...
        val allExp = expActor.ask(GetAllExperiments(page, max)).mapTo[AllExperiments]
      ...
      get {
        logger.debug("GET on /experiments, should return all experiments")
        val allExp = expActor.ask(GetAllExperiments()).mapTo[AllExperiments]
        onSuccess(allExp) { exps ⇒
          complete(OK -> exps)
        }
      }
    ...
    pathPrefix(Segment) { experiment ⇒
      path("transforms") {
        get {
          ...
          onSuccess(expActor.ask(GetTransforms(experiment)).mapTo[TransformsForExperiment]) {
          ...
          pathPrefix(Segment) { transform ⇒
            path("selectable") {
              get {
                ...
                val sele4Tran = expActor.ask(GetSelectable(experiment, transform)).mapTo[Option[BunchOfSelectables]]
        ...
        path("tots") {
          ...
          get {
            ...
            onSuccess(expActor.ask(GetToTs(experiment)).mapTo[ToTsForExperiment]) {
        ...
        pathPrefix("published") {
          path(Segment) { p ⇒
            delete {
              ...
              val delP = expActor.ask(RemovePublished(experiment, p)).mapTo[DefaultFeedback]
              onSuccess(delP) {
            ...
            pathEnd {
              get {
                ...
                val pub = expActor.ask(GetPublished(experiment)).mapTo[Published]
                onSuccess(pub) {
            ...
            post {
              ...
              delete {
                ...
                entity(as[RmFile]) { rmf ⇒
                  val saved: Future[RemoveFileFeedback] = expActor
                ...
                delete {
                  ...
                  entity(as[RmFile]) { rmf ⇒
                    val saved: Future[RemoveFileFeedback] =
                      expActor.ask(RemoveUploadedRawFile(experiment, rmf.fileName)).mapTo[RemoveFileFeedback]
                    onSuccess(saved) {
        ...
        pathPrefix("files") {
          path("meta") {
            get {
              ...
              onSuccess(expActor.ask(InfoAboutMetaFiles(experiment)).mapTo[FilesInformation]) {
            ...
            path("user_raw") {
              get {
                ...
                onSuccess(expActor.ask(InfoAboutUserRawFiles(experiment)).mapTo[FilesInformation]) {
                  case FilesInformation(ffi) ⇒ complete(OK -> ffi)
                }
              }
            } ~
            path("raw") {
              get {
                ...
                onSuccess(expActor.ask(InfoAboutRawFiles(experiment)).mapTo[FilesInformation]) {
                  case FilesInformation(ffi) ⇒ complete(OK -> ffi)
                }
              }
            } ~
            pathEnd {
              get {
                ...
                onSuccess(expActor.ask(InfoAboutAllFiles(experiment)).mapTo[AllFilesInformation]) {
          ...
          pathEnd {
            post {
              ...
              entity(as[AddDesign]) { des ⇒
                val saved: Future[AddDesignFeedback] = expActor.ask(des).mapTo[AddDesignFeedback]
                onSuccess(saved) {
        ...
        path("hide") {
          post {
            ...
            entity(as[WithToken]) { wt ⇒
              val changed: Future[HideUnHideFeedback] = expActor.ask(Hide(experiment)).mapTo[HideUnHideFeedback]
        ...
        path("unhide") {
          post {
            ...
            entity(as[WithToken]) { wt ⇒
              val changed: Future[HideUnHideFeedback] = expActor.ask(Unhide(experiment)).mapTo[HideUnHideFeedback]
              onSuccess(changed) {
        ...
        pathPrefix("logs") {
          parameters('page ? 0, 'max ? 100) { (page, max) ⇒
            ...
            val getLogs = expActor.ask(ReadLogs(experiment, page, max)).mapTo[InfoLogs]
            ...
            post {
              ...
              entity(as[AddExpProps]) { props ⇒
                val saved: Future[AddedPropertiesFeedback] =
                  expActor.ask(AddExpProperties(experiment, props.properties))
              ...
              delete {
                ...
                entity(as[RmExpProps]) { props ⇒
                  val removed: Future[RemovePropertiesFeedback] =
                    expActor.ask(RemoveExpProperties(experiment, props.properties))
            ...
            put {
              ...
              entity(as[ChangeDescription]) { desc ⇒
                val saved: Future[DescriptionChangeFeedback] =
                  expActor.ask(ChangeDescriptionOfExperiment(experiment, desc.description)).mapTo[DescriptionChangeFeedback]
                onSuccess(saved) {
            ...
            post {
              ...
              entity(as[CloneExperimentNewProps]) { exp ⇒
                val saved: Future[AddExperimentResponse] =
                  expActor.ask(CloneExperiment(experiment, exp)).mapTo[AddExperimentResponse]
                onSuccess(saved) {
        ...
        pathEnd {
          get {
            ...
            val getExp = expActor.ask(GetExperiment(experiment)).mapTo[ExperimentFoundFeedback]
            onSuccess(getExp) {
            ...
            delete {
              ...
              val delExp = expActor.ask(DeleteExperiment(experiment)).mapTo[DeleteExperimentFeedback]
              onSuccess(delExp) {
      ...
      pathEnd {
        post {
          ...
          entity(as[AddExperiment]) { addExp ⇒
            val saved: Future[AddExperimentResponse] = expActor.ask(addExp).mapTo[AddExperimentResponse]
            onSuccess(saved) {
              case addExp: AddedExperiment ⇒ complete(Created -> addExp)
              case FailedAddingExperiment(msg) ⇒ complete(Conflict -> ErrorMessage(msg))
            }
